...
impl<'data> SystemImage<'data> {
    ...
    fn add_app_segments(
        ...
        for segment in &app.segments {
            ...
            // ============================================================================
            // FIX: Adjust segment p_vaddr to match first section's sh_addr
            // ============================================================================
            //
            // BUG: On ARMv7-M targets, the original app ELF's LOAD segment has p_vaddr
            // aligned to a power-of-2 boundary (e.g., 0x28000), but the first section
            // starts at a higher address (e.g., 0x28200). ELF loaders use p_vaddr to
            // determine where to load the segment, so code ends up at the wrong address.
            //
            // EXAMPLE:
            //   - Original segment: p_vaddr = 0x28000, contains .code at offset 0x200
            //   - Section .code: sh_addr = 0x28200 (preserved by earlier fix)
            //   - Loader sees: "load this segment to 0x28000"
            //   - Result: .code gets loaded to 0x28000, but symbols point to 0x28200
            //   - CPU jumps to 0x28200 -> executes wrong code -> MemManage fault
            //
            // FIX: Set p_vaddr = first_section.sh_addr so loader puts code at correct address.
            // This works because we also preserve section sh_addr values (above), so
            // everything aligns: segment p_vaddr = section sh_addr = symbol addresses.
            // ============================================================================
